...
function Maps() {
  ...
  const [center, setCenter] = useState({
    lat: 41.38201718772406,
    lng: 2.140611050113362,
  });
  ...
  const getAllFonts = () => {
    axios.get(`${API_URL}/fonts`).then((fontsData) => {
      setFonts(fontsData.data);
    });
  };

  const getAllLavabos = () => {
    axios.get(`${API_URL}/lavabos`).then((lavabos) => {
      setLavabos(lavabos.data);
    });
  };
  ...
  return (
    <body>
      ...
      <div className="flexBotons" style={{ backgroundColor: "#1F4690" }}>
        <Button style={{ color: "white" }} onClick={() => setShowLab(!showLab)}>
          {showLab ? "Hide public WC" : "Show public WC"}
          <img
            style={{ marginLeft: "15px", backgroundColor: "" }}
            src={wcIconWhi}
            width={28}
            height={35}
            alt="wc"
        ...
        <Button style={{ color: "white" }} onClick={() => setShowFon(!showFon)}>
          {showFon ? "Hide drinking fountains" : "Show drinking fountains"}
          <img
        ...
        <Button style={{ color: "white" }} onClick={() => setShowPis(!showPis)}>
          {showPis ? "Hide Swimming Pools" : "Show Swimming Pools"}
          <img
            style={{ marginLeft: "15px" }}
            src={piscIconWhi}
            width={30}
        ...
        {fonts !== 0 &&
          ...
          fonts.map((font) => {
            ...
            return (
              ...
              >
                <Popup
                  className="custom-popup leaflet-popup"
                  style={{ textAlign: "center", position: "center" }}
                >
                  {!nom && <h3>Drinking fountain</h3>}
                  <h3>{nom.toUpperCase()}</h3>

                  {comments.length !== 0 &&
                    comments.map((comment) => {
                      const { photo, title, updatedAt, rating, content, user } =
                        comment;

                      return (
                        <>
                          <section
                            className="leaflet-popup-content-wrapper leaflet-popup-content"
                            style={{
                              margin: "10px 5px",
                              padding: "10px",
                              textAlign: "center",
                          ...
                          {editC && isLoggedIn && (
                            <Formulari
                              ...
                              knowPhoto={knowPhoto}
                              id={comment._id}
                              editData={(photo, title, content, rating)}
                            />
                          )}
                        </>
                      );
            ...
            return (
              ...
              >
                <Popup className="request-popup">
                        ...
                        <>
                          ...
                          {editC && isLoggedIn && (
                            <Formulari
                              ...
                              id={comment._id}
                              knowPhoto={knowPhoto}
                              editData={(photo, title, content, rating, _id)}
                            />
                          )}
                        </>
                      );
                  ...
                  <Button
                    color="success"
                    style={{ border: "solid 1.5px green" }}
                    onClick={() => setShowForm(!showForm)}
                  >
                    {showForm ? "Hide" : "Comment"}
                  </Button>
      ...
      <div style={{ display: "flex", marginTop: "35%" }}>
        ...
        >
          ...
          <t style={{ marginTop: "-10px" }}>
            <t style={{ textDecoration: "underline" }}>Right click</t> to know
            your location. In mobile{" "}
            <t style={{ textDecoration: "underline" }}>hold tap</t>.
          </t>
        </div>
      </div>
    </body>
